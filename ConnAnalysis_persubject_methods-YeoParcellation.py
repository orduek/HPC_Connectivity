...
masker = NiftiLabelsMasker(labels_img=atlas_filename, standardize=True,smoothing_fwhm = 6,
                        memory="/media/Data/nilearn",high_pass=.008 , low_pass = .09, t_r=1, verbose=5)
...
plotting.plot_matrix(np.average(ket2_ket1 ,axis = 0), labels=atlas_labes['Label Name'], colorbar=True,
                    vmax=0.9, vmin=-0.9)
...
import pandas as pd
adDat = pd.DataFrame(ad, columns=atlas_labes[1:,0],index=atlas_labes[1:,0])
print(adDat)
